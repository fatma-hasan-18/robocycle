...
`lib/auth/routes.ts` تحدّد ما يتطلّب حسابًا كاملًا (`/dashboard`، `/profile`).
الضيف يمرّ في كل ما عداها، ويُوجَّه إلى `/enter?upgrade=1` عند محاولة دخول مسار
محمي.

`/rewards` **ليست محمية عمدًا**: الضيف يملك نقاطًا حقيقية في القاعدة ويستطيع
الاستبدال، فحجب الصفحة عنه يقطع المسار الأساسي للتطبيق. الحماية الفعلية في
قاعدة البيانات (RLS والمشغّلات) لا في إخفاء الصفحة.

**صفحة المكافآت (`/rewards`)** تعرض الرصيد والمخزون ولوحة الصدارة، وتستدعي
`redeem_reward`. رصيد المستخدم المسجَّل يأتي من الخادم عبر `get_my_summary`،
ورصيد الضيف يُجلب من المتصفح عبر `wallet_balance` لأن مفتاح محفظته في
`localStorage` ولا يستطيع الخادم قراءته.
